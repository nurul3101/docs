...
#### Overfetching of data due to Incorrect/Missing <inlinecode>[where](/orm/reference/prisma-client-reference#where)</inlinecode> or <inlinecode>[select](/orm/reference/prisma-client-reference#select)</inlinecode> clause

In certain cases, a large number of records or fields are unintentionally fetched, which results in exceeding the 5MB limit.

This could happen when the `where` clause in the query is incorrect or entirely missing.

Additionally, if you only need to use specific fields from the table, then you can use the select clause in your queries to prevent unnecessary data from being fetched.
